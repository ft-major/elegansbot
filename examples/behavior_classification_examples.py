...
def get_smoothed_parameter_from_kymogram(theta_t_, dt):
    ...
    for name in p__raw_.dtype.names:
        d__[name] = savgol_filter(p__raw_[name], window_length=int((1 / dt) / 4) * 2 + 1, polyorder=2,
                                  deriv=1)  # time window of 0.5 sec length
...
def plot_ethogram_behavior_log(ax, behavior_log_, n_t):
    ...
    for i in range(len(behavior_log_)):
        ...
        if (i_snapshot_2 - i_snapshot) / n_t > .1:
            ax.text(x=(i_snapshot + i_snapshot_2) / 2, y=.5, s=behavior[0].upper() + behavior[1:], color=(1, 1, 1),
...
def plot_kymogram_parameter_n_behavior_log(ax, dt, theta_t_, theta_sine_t_, p__raw_, p__, d__, behavior_log_,
                                           y_0_turn=-.07, should_plot_diff=True, fixed_yxratio=.2):
    fig = ax.get_figure()
...
def plot_process_behavior_classification(
    ...
    behavior_log_ = get_behavior_log_(p__, d__, y_0_turn=y_0_turn)
    plot_kymogram_parameter_n_behavior_log(ax, dt, theta_t_, theta_sine_t_, p__raw_, p__, d__, behavior_log_, y_0_turn,
                                           should_plot_diff=False)
    set_bbox_inches_tight(fig)


# %%
def plot_heatmap_rod_force(env, f_s_max=None, i_snapshot_arrow_=None, fixed_yxratio=.2, y_title=-.4,
                           should_quick_draw=False, verbose=False, dpi=72, figsize=(7, 1.5)):
    """
    ...
    plot_color_wheel(
        ax,
        yticklabels=(
            rf'${{{f_s_max / 2:.0f}}} \mathrm{{pN}}$' if np.log10(
                f_s_max / 2) >= 0 else rf'${{{f_s_max / 2}}} \mathrm{{pN}}$',
...
try:
    ...
    env = utils.Worm(scale_friction=.01, angle_init=2 * np.pi * (.43))
...
try:
    ...
    env = utils.Worm(scale_friction=.01, angle_init=2 * np.pi * (-.015), polarity_clockwise=True)
